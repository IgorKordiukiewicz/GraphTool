...
void EditPanel::saveGraphToFile(const std::string& name)
{
	...
	for (const GraphNodeShape& nodeShape : graphEditor.getNodesShapes()) {
	...
	file << '\n';
	for (const Edge& edge : graph.getDirectedEdges()) {
		file << edge.a << " " << edge.b << " " << edge.weight << " ";
	}
	file << '\n';
	for (const Edge& edge : graph.getUndirectedEdges()) {
		file << edge.a << " " << edge.b << " " << edge.weight << " ";
	}
...
void EditPanel::loadGraphFromFile(const std::string& name)
{
	...
	while (std::getline(file, line)) {
		...
		if (lineIdx == 0) {
			...
			// Create nodes
			const std::vector<int> nodes{ splitLine() };
			...
			const std::vector<int> directedEdges{ splitLine() };
			std::getline(file, line);
			const std::vector<int> undirectedEdges{ splitLine() };
